...
// Trae TODOS los videos de una playlist pública de YouTube usando la
// YouTube Data API v3 (necesita window.YOUTUBE_API_KEY en firebase-config.js).
// Avanza página por página (50 videos por página) hasta traerlos todos.
export async function fetchPlaylistVideos(playlistId, onProgress){
  const apiKey = window.YOUTUBE_API_KEY;
  if(!apiKey){
    throw new Error('Falta configurar YOUTUBE_API_KEY en firebase-config.js');
  }
  let videos = [];
  let pageToken = '';
  do{
    const url = 'https://www.googleapis.com/youtube/v3/playlistItems?part=snippet&maxResults=50&playlistId='
      + encodeURIComponent(playlistId) + '&key=' + apiKey
      + (pageToken ? '&pageToken=' + pageToken : '');
    const res = await fetch(url);
    const data = await res.json();
    if(data.error){
      throw new Error(data.error.message || 'Error al consultar la YouTube Data API');
    }
    (data.items || []).forEach(item=>{
      const sn = item.snippet;
      const videoId = sn && sn.resourceId && sn.resourceId.videoId;
      const title = sn && sn.title;
      // Se saltan los videos privados o eliminados de la playlist
      if(videoId && title && title !== 'Private video' && title !== 'Deleted video'){
        videos.push({ videoId, name: title });
      }
    });
