...
function ytKeyOk(){
  return window.YOUTUBE_API_KEY && !window.YOUTUBE_API_KEY.includes('PEGA_AQUI');
}

// Trae los títulos reales de uno o varios videoIds usando la API oficial de YouTube.
// Devuelve un objeto { videoId: 'Título', ... }
export async function fetchVideoTitles(videoIds){
  if(!ytKeyOk() || !videoIds.length) return {};
  const titles = {};
  for(let i=0; i<videoIds.length; i+=50){
    const chunk = videoIds.slice(i, i+50);
    const url = `https://www.googleapis.com/youtube/v3/videos?part=snippet&id=${chunk.join(',')}&key=${window.YOUTUBE_API_KEY}`;
    const res = await fetch(url);
    const data = await res.json();
    if(data.error) throw new Error(data.error.message || 'Error de YouTube API');
    (data.items || []).forEach(item=>{ titles[item.id] = item.snippet.title; });
  }
  return titles;
}

// Trae TODOS los videos de una playlist pública (con paginación automática).
// Devuelve un arreglo de { videoId, title }
export async function fetchPlaylistVideos(playlistId){
  if(!ytKeyOk()) throw new Error('Falta configurar YOUTUBE_API_KEY en firebase-config.js');
  const videos = [];
  let pageToken = '';
  do{
    const url = `https://www.googleapis.com/youtube/v3/playlistItems?part=snippet&maxResults=50&playlistId=${playlistId}&key=${window.YOUTUBE_API_KEY}` + (pageToken ? `&pageToken=${pageToken}` : '');
    const res = await fetch(url);
    const data = await res.json();
    if(data.error) throw new Error(data.error.message || 'Error de YouTube API');
